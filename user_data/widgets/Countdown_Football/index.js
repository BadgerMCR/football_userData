module.exports = {
	...
	// 	}
	// ],
	vue_template: '<div class="countdown"><h1 style=" display: inline; width: 15px; font-size: 1.1em; position:absolute; top:79px; left:123px; margin-left: 103px;">{{teamAname}}</h1> <div class="colorA" :style="solidColorStyle"> </div><h2 style="display: inline-block; text-align: center; width:50px; font-size: 1em; position:absolute; top:90px; left:316px; margin-left: 7px" >{{scoreA}}</h2><h2 style="display: inline-block; text-align: center; width:50px; font-size: 1em; position:absolute; top:90px; left:372px; margin-left: 10px">{{scoreB}}</h2> <div class="colorB" :style="solidColorStyle"></div> <h1 style=" display: inline; font-size: 1.1em; position:absolute; top:79px; left:453px;">{{teamBname}}</h1><span :style="cssVars" class="counter">{{counter}}</span></div>',
	vue_component: getFile("component.js").toString(),
	css: ".countdown{font-variant-numeric: tabular-nums; background: url(/mediafiles/score_board.png); background-repeat:no-repeat; padding: 83px 113px!important;} .counter{display: inline-block; margin-left: 5px; position:absolute; top:83px; left:542px; padding:6.5px 10px;} .colorA {width: 50%; height: 50px; position: absolute; top: 50px; left: 200px; } .colorB {width: 100%; height: 100px; position: relative; top: 100px; left: 1000px; }"
}
